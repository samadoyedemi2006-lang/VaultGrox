...
export default function Dashboard() {
  ...
  const stats = [
    ...
    {
      label: "Daily ROI Earned (15%)",
      value: `₦${(data?.totalRoiEarned || 0).toLocaleString()}`,
      icon: <ArrowDownToLine className="h-5 w-5" />,
    },
    {
      label: "Active Investments",
      value: String(data?.activeInvestments || 0),
      icon: <Activity className="h-5 w-5" />,
    },
    {
      label: "Referral Earnings",
      value: `₦${(data?.referralEarnings || 0).toLocaleString()}`,
  ...
  return (
    <DashboardLayout>
      <PageHeader
        title="Dashboard"
        subtitle={`Welcome back, ${user?.fullName || "Investor"}!`}
      />
